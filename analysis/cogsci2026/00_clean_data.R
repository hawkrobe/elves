...
# Next-nearest = closer of ±45° neighbors (reconstruct from geometry; do not use jsPsych)
# Circular distance; then pick neighbor_plus or neighbor_minus whichever is closer to target angle.
circ_dist <- function(a, b) pmin(abs(a - b), 360 - abs(a - b))
...
d_production_cleaned <- d_production |> 
  mutate(
    ...
    dist_plus = circ_dist(angle, neighbor_plus),
    dist_minus = circ_dist(angle, neighbor_minus),
    next_nearest_angle = if_else(dist_plus < dist_minus, neighbor_plus, neighbor_minus)
  ) |> 
